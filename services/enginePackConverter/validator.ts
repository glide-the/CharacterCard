...
import type { DeterministicMappingResult, TraceEntry } from "./mapper/deterministicMapper";
import type { LlmProtocolMappingResult } from "./mapper/llmProtocolMapper";
...
function mergeTraceMap(
  deterministic: DeterministicMappingResult,
  llm?: LlmProtocolMappingResult
): TraceEntry[] | undefined {
  const base = deterministic.extensions?.traceMap ?? [];
  const llmTraces = llm?.traces ?? [];
  const mapped = llmTraces
  ...
  const combined = [...base, ...mapped];
  return combined.length ? combined : undefined;
}

function normalizePath(filePath: string, packageRoot?: string): string {
  if (path.isAbsolute(filePath)) return filePath;
  if (!packageRoot) return filePath;
  return path.join(packageRoot, filePath);
}

function uniqueIds(ids: string[]): { duplicates: string[]; missing: number } {
...
function mergeReviewQueue(...queues: Array<ReviewQueueItem[] | undefined>): ReviewQueueItem[] {
  const combined: ReviewQueueItem[] = [];
  for (const queue of queues) {
    if (!queue) continue;
    for (const item of queue) combined.push(item);
  }
  return combined;
}
...
export function buildEvaluationPack(
  ...
  return {
    ...
    promptAssets: llm?.promptAssets ?? deterministic.promptAssets,
    warnings: [...deterministic.warnings, ...(llm?.warnings ?? [])],
    extensions: {
      ...deterministic.extensions,
      traceMap: mergeTraceMap(deterministic, llm),
    },
  };
}

export async function validateEvaluationPack(
  deterministic: DeterministicMappingResult,
  llm?: LlmProtocolMappingResult,
  ...
  const reviewQueue: ReviewQueueItem[] = mergeReviewQueue(deterministic.reviewQueue, llm?.reviewQueue);
