 ...
 */
function schemaToPromptDescription(geminiSchema, indent = 0) {
    ...
    if (geminiSchema.type === Type.ARRAY && geminiSchema.items) {
        return `[\n${schemaToPromptDescription(geminiSchema.items, indent + 1)}\n${prefix}]`;
    }
 ...
 */
export const generateContent = async (config, request, taskConfig) => {
 ...
 */
export async function* generateStream(config, request, taskConfig) {
    validateConfig(config);
    // --- Gemini Stream ---
    if (config.provider === 'gemini') {
        const apiKey = config.gemini?.apiKey || process.env.API_KEY;
...
export async function executeWithRetry(fn, taskConfig) {
    let lastError = null;
    for (let attempt = 0; attempt <= taskConfig.maxRetries; attempt++) {
        try {
            const result = await Promise.race([
                fn(),
                new Promise((_, reject) => setTimeout(() => reject(new Error('Task timeout')), taskConfig.timeoutMs)),
            ]);
            return result;
        }
        catch (error) {
